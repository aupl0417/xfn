...
//                break;
    ...
    public function saoma(){
        $redirect_url = 'http://api.' . config('url_domain_root') . '/car_v1/callback';
        $state   = input('id', 0, 'intval');
        $service = model('Wechat', 'service');
        $url     = $service->oauth2($redirect_url, $state);
        header("Location:" . $url);
    }

    public function callback(){
        $code      = input('code', '', 'htmlspecialchars');
        $id        = input('state', '', 'htmlspecialchars');
        $service   = model('Wechat', 'service');

//        $tokenInfo = action('Home/getAccessTokenByCode', ['code' => $code]);
//        $userInfo  = $this->getUser($tokenInfo['access_token'], $tokenInfo['openid']);
        $tokenInfo = $service->getAccessTokenByCode($code);
        $userInfo  = $service->getUser($tokenInfo['access_token'], $tokenInfo['openid']);
        ...
        try{
            ...
            if(!$wuser){
                $data = [
                    ...
                    'w_subscribe'     => $userInfo['subscrib'],
                    'w_uninid'        => $userInfo['uninid'],
                    'w_subscribeTime' => $userInfo['subscribeTime'],
                ];
            ...
            $result = Db::name('business_opportunity')->where(['bo_id' => $id])->update(['bo_wid' => $wId]);
            if($result === false){
